...
さらに2,3において、「実数倍かどうか」とは、
点Pを「与えられたベクトルを使って一意に表せるかどうか」と同一である。

2の実数倍($\vec{a} = k\vec{b}$)の場合、点Pは
...
といった具合に与えられたベクトルを使って複数の表し方が存在する。(無限に存在する)

一方3の実数倍でない($\vec{a} \neq k\vec{b}$)の場合、点Pは、$\vec{a}$, $\vec{b}$を使って一意にしか表せない。

```
$\vec{p} = k_1\vec{a} + k_2\vec{b}
```

つまり$\vec{a}$, $\vec{b}$ は互いに独立していると言える。
...
与えられた２つのベクトルが
ゼロベクトルでなくかつ
互いに独立しているベクトルの一次結合によって表されるベクトルの集合である。
